...
3. 하둡 ( 에코시스) 시스템은 하둡 프레임워크를 이루고 있는 다양한 서브 프로젝트들의 모임이다.
...
SQL 데이터베이스는 데이터 구조가 고정되어 있고, NoSQL 데이터베이스는 유연하다.
확장성에서 SQL 데이터베이스는 수직적으로 확장되고, NoSQL 데이터베이스는 수평적으로 확장된다.
속도에서 SQL 데이터베이스는 복잡한 쿼리에 적합하고, NoSQL 데이터베이스는 대규모 데이터 처리에 최적화 되어있다.
