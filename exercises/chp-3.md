...
1. 데이터 구조는 표현하려고 하는 데이터(또는 데이터 모델)의 정적 측면을, 데이터 연산은 데이터 (또는 데이터 모델)의 동적 측면을 명세한다. (참)
2. 관계형 데이터 모델(relational data model)은 이론적으로는 릴레이션(relation) 이라는 수학적 집합 개념에 기초하고 있다. (참)
3. 속성 이름이 다르더라도 도메인이 같다면 속성들의 값 비교가 가능하며 의미적으로도 옳다. (참)
4. 속성이 없는 릴레이션은 존재할 수 있지만 투플이 없는 릴레이션은 존재할 수 없다. (거짓)
5. 투플들의 순서만 다른 두 릴레이션은 같은 릴레이션이 된다. (참)
6. 릴레이션은 다중 값 속성은 허용하나 복합 속성은 허용하지 않는다. (거짓)
7. 모든 릴레이션은 최소한 하나 이상의 후보키를 가진다. (참)
8. 외래키는 널 값을 가질 수 없다. (거짓)
9. 외래키와 참조하는 기본키 사이에는 속성 이름과 대응하는 속성들의 수가 같고 도메인은 서로 일치해야 한다. (참)
...
릴레이션 스키마: 데이터베이스의 구조를 정의하고, 테이블의 이름과 속성 이름, 도메인 등을 포함
릴레이션 인스턴스: 특정 시점에서의 데이터를 나타내,고 릴레이션에 포함된 튜플들의 집합

**19 관계형 데이터 모델의 무결성 제약 조건 4가지를 쓰고 간략히 설명하시오.**
도메인 무결성: 속성 값은 정의된 도메인에 속해야 한다
개체 무결성: 기본키는 중복되거나 널 값을 가질 수 없다
참조 무결성: 외래키 값은 참조 릴레이션의 기본키와 일치하거나 널이어야 한다
사용자 정의 무결성: 비즈니스 규칙에 따른 제약 조건

**20 후보키와 슈퍼키, 기본키 사이의 관련성에 대해 설명하시오.**
슈퍼키: 속성 집합 중 유일성을 만족하는 모든 키
후보키: 슈퍼키 중 최소성을 만족하는 키
기본키: 후보키 중 하나를 선택하여 지정한 키

**21 후보키 중에서 기본키를 선정하기 위한 선정 기준에 대해 설명하시오.**
유일성과 최소성을 만족해야 한다
속성 값이 널 값을 가지지 않아야 한다
길이가 짧고 변경 가능성이 낮은 속성을 선택한다
