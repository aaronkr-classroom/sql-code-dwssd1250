...
문서형| JSON, BSON 기반으로 데이터를 저장하며 비정형 데이터를 처리에 적합함
키-값형| 간단한 키-값 쌍으로 데이터를 저장하며 빠른 조회 성능을 제공
컬럼형| 대규모 분산 데이터 처리에 적합하며 테이블 기반이지만 컬럼 중심으로 설계됨
그래프형| 데이터 간 관계를 저장하고 쿼리하는 데 최적화됨

**11 몽고DB와 관계형 데이터베이스를 비교 • 설명하시오.**
데이터 모델| 몽고DB:문서형 모델               관계형DB:테이블 기반 모델
스키마|      몽고DB:동적 스키마               관계형DB:고정된 스키마
확장성|      몽고DB:수평적 확장               관계형DB:수직적 확장
트랜잭션|    몽고DB:부분적 지원               관계형DB:완전한 ACID 지원
쿼리 언어|   몽고DB:JavaScript 기반 명령어    관계형DB:SQL
적합성|      몽고DB:비정형/반정형 데이터 분석  관계형DB:정형 데이터 관리
