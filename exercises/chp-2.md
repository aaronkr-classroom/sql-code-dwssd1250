...
**02 데이터베이스 관리자(DBA) 의 역할에 관한 설명으로 거리가 먼 것은?** 답: 3
...
**03 데이터베이스 관리자(OBA)가 수행해야 하는 주요 역할로 거리가 먼 것은?** 답: 3
...
**04 DBMS 의 필수 기능과 관런 데이터 언어, 관련 사용자의 종류가 바르게 연결된 것은?** 답: 4
...
**06 DBMS 의 제어 기능에 대한 설명으로 옳지 않은 것은?** 답: 2
...
**07 DBMS 의 주요 구성 요소에 해당하지 않는 것은?** 답: 4
...
**08 데이터베이스와 DBMS 유형을 분류하는 데이터 모델에 속하지 않는 것은?** 답: 4
...
**09 시스템 데이터베이스(시스템 카탈로그)에 대한 설명으로 옳지 않은 것은?** 답: 3
...
**10 응용 프로그램에 영향을 주지 않고 구조를 변경시컬 수 있는 데아터베이스 특성은?** 답: 1
...
**11 3-단계 데이터베이스 스키마 구조에 해당하지 않는 것은?** 답: 1
...
**13 DBMS의 필수 기능 중에셔 하나의 통합된 데이터베이스 구조를 명세하고 생성, 삭제, 수정함으로써 여러 응용 프로그램이 요구하는 네이터베이스 구조를 지원하는 것은 어떤 기능에 포함되는가?** 답: 1
...
**14 다음 문장이 참이면 0, 거짓이먼 x 를 하시오.** 답: 
...
8. 데이터베이스를 구성하는 모든 데이불들의 구조를 명세한 것이 (테이블 스키마)이다.
...
데이터 보안 관리, 백업 및 복구 계획, 데이터베이스 설계 및 최적화, 사용자 권한 관리


**17 DBMS 의 3가지 필수 기능에 대해 설명하시오.**
정의 기능: 데이터베이스 구조와 제약 조건 정의
조작 기능: 데이터 검색, 삽입, 삭제, 수정
제어 기능: 무결성, 보안, 동시성 제어

**18 데이터 모델에 따른 DBMS 의 역사에 대해 간략히 설명하시오.**
계층형 모델: 1960년대- 데이터가 계층적 구조로 연결
망형 모델: 1970년대- 데이터 관계를 그래프로 표현
관계형 모델: 1980년대- 데이터를 표 형태로 관리

**19 데이터 사전(data dictionary)과 데이터 디렉토리 (data directory)의 차이점을 설명하시오.**
데이터 사전: 메타데이터 저장소, DBMS에서 사용
데이터 디렉토리: 물리적 파일 경로와 관련된 정보

**20 다양한 수준의 추상화와 테이터 독립성율 제공하기 위하여 대이터베이스 관리 시스템 (DBMS)에서 지원하는 3 단계 스키마 구조에 대해셔 간략히 설명하시오.**
외부 스키마: 사용자 관점
개념 스키마: 전체적인 데이터베이스 구조
내부 스키마: 물리적 데이터 저장 방식
